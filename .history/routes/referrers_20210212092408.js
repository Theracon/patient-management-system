...
router.get("/referrers/:id/pending", middleware.isUserLoggedIn, middleware.isReferrerAuthorized, function(req, res) {
    User.findOne({ typeOfUser: "referrer", _id: req.params.id }, function(err, user) {
        if(!err) {
            if(user) {
                if(user.role === 0) {
                    res.render("referrers/pending");
                } else {
                     res.redirect("/referrers/" + user._id + "/dashboard");
                }
            }
