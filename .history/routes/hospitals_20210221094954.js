...
router.get("/hospitals/:username/alt", middleware.isUserLoggedIn, middleware.isHospitalProfileCreated, function(req, res) {
    User.findOne({ typeOfUser: "hospital", username: req.params.username }, function(err, user) {
        if(!err) {
            if(user) {
...
router.post("/hospitals/:username/alt", middleware.isUserLoggedIn, middleware.isHospitalProfileCreated, function(req, res) {
    User.findOne({ typeOfUser: "hospital", username: req.params.username }, function(err, user) {
        if(!err) {
            if(user) {
                req.body.procedures.forEach(function(procedure) {
                    user.hospitalDetails.procedures.push(procedure);
                });

                for(var i = 0; i < req.body.procedure.length; i++) {
                    ser.hospitalDetails.procedures.push(req.body.procedures[i]);
                }
                user.role = 0.75;
                user.save(function(err, user) {
                    if(err) {
            ...
            if(hospital) {
                ...
                Patient.findOne({ accession_number: accessionNumber }, function(err, patient) {
                    ...
                    User.findOne({ _id: patient.referrer_id }, function(err, referrer) {
                        ...
                        // Set amount paid and calculate referrer commission
                        ...
                        patient.save(function(err, patient) {
                            ...
                            referrer.save(function(err, referrer) {
                                ...
                                hospital.save(function(err, hospital) {
                                    ...
                                    return res.redirect("/hospitals/" + hospital.username + "/dashboard");
                                })
                            }); 
                        });
                    });
