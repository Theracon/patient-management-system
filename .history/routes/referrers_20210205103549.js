    ...
    User.findOne({ typeOfUser: "referrer", _id: req.params.id }, function(err, user) {
        if(!err) {
            if(user) {
                ...
                if(user.referrerDetails.unread_notifications > 0) {
                    user.referrerDetails.unread_notifications--;
                }
                // Find notification and change its status
                user.referrerDetails.notifications
                // Save updated user
