...
router.get("/hospitals/:username/procedures/edit", middleware.isUserLoggedIn, middleware.isReferrerAuthorized, function(req, res) {
    User.findOne({ typeOfUser: "hospital", username: req.params.username }, function(err, user) {
        if(!err) {
            if(user) {
...
router.put("/hospitals/:username/procedures", middleware.isu)
